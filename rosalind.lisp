...
(defun rosalind-find-restriction-sites ()
  (let ((dna-string (second (first (read-fasta-lines "rosalind_revp.txt")))))
...
(defun rosalind-print-restriction-sites ()
  (iter (for (pos length) in (sort (rosalind-find-restriction-sites) #'< :key #'first))
	(format t "~a ~a~%" (1+ pos) length)))
    ...
    (iter (for i from 1 to (1- (length data)))
	  (iter (for k from i downto 1)
		(symbol-macrolet ((data-k (elt data k))
		  ...
		  (incf swaps-count))))
    swaps-count))
(defun rosalind-count-insertion-sort-swaps ()
  (let* ((data (second (read-file-lines "rosalind_ins.txt")))
	 (unsorted-data (mapcar #'parse-integer (split-sequence:split-sequence #\Space data))))
    (count-insertion-sort-swaps (coerce unsorted-data 'vector))))

(defun list-permutations (list)
  (when (not (null list))
    (iter (for i in list)
	  (collect (mapcar #'(lambda (r) (cons i r)) (list-permutations (remove i list)))))))
(defun print-all-permutations (table)
  (iter (for permutation in (list-permutations table))
	(format t "~{~a~^ ~}~%" permutation)))
(defun rosalind-permutations ()
     (let* ((list-length (parse-integer (first (read-file-lines "rosalind_perm.txt"))))
	    (table (iter (for i from 1 to list-length)
			 (collect i))))
       (format t "~a~%" (reduce #'* (loop for i from 1 to list-length collect i)))
       (print-all-permutations table)))
