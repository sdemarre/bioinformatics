...
(define-rosalind-problem :rna
  ...
  (with-single-input-line (dna)
    (write-single-output-line (cl-ppcre:regex-replace-all "T" dna "U"))))

(define-rosalind-problem :revc
  "complementing a strand of dna"
  (with-single-input-line (dna)
    (write-single-output-line (reverse-complement dna))))
...
(define-rosalind-problem :hamm
  ...
  (with-input-lines (lines)
    (let* ((dna1 (first lines))
	   (dna2 (second lines)))
      (write-single-output-line (hamming-distance dna1 dna2)))))
...
(define-rosalind-problem :fibd
  ...
  (with-single-input-line (problem-data)
   (destructuring-bind-integers (generations pair-lifetime) problem-data
     (write-single-output-line (mortal-rabits generations pair-lifetime)))))
...
(define-rosalind-problem :prtm
  ...
  (with-single-input-line (problem-data)
    (write-single-output-line
      (format nil "~f" (protein-mass problem-data)))))
...
(defun make-long-example (dna-string-length number-reads read-length)
  (let ((s (make-random-dna-string dna-string-length)))
    (iter (for reads = (make-random-substrings s number-reads read-length))
	  (for prefix-matches = (compute-best-prefix-matches reads))
	  (when (= (hash-table-count prefix-matches) (1- number-reads))
	    (return (list s reads prefix-matches))))))
  ...
  (let* ((dna-strings (coerce dna-strings 'vector))
    ...
    (iter (while (gethash current-string-index best-prefix-matches))
	  (let* ((current-string (elt dna-strings (1- current-string-index)))
		 (successor-info (gethash current-string-index best-prefix-matches))
		 (new-string-index (getf successor-info :successor-idx)))
	    (if (gethash new-string-index best-prefix-matches)
		(let ((length (- (length current-string) (getf successor-info :suffix-length))))
		 (collect (subseq-of-length current-string 0 length)))
		...
		(unless (eq dna-string-1 dna-string-2)
		  (iter (for substr-len from (length dna-string-1) downto (floor (length dna-string-1) 2))
			(when (matching-prefix-postfix-p dna-string-1 substr-len dna-string-2)
			  (setf (gethash idx2 best-prefix-match) (list :successor-idx idx1 :suffix-length substr-len))
			  (return))))))
    best-prefix-match))
...
(define-rosalind-problem :sset
  ...
  (with-single-input-line (elements)
    (write-single-output-line (mod (expt 2 (parse-integer elements)) 1000000))))
