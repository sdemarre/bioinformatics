...
(defun get-uniprot-choice (location &optional retrying)
  ...
  (multiple-value-bind (fasta-string http-status info)
    ...
    (cond ((= 200 http-status)
	   (fasta-combine-lines (split-sequence:split-sequence #\Newline fasta-string)))
	  ((= 404 http-status)
	   (if retrying
	       (error "not found, even after waiting patiently and retrying")
	       (progn
		 (format t "  whoops... 404. maybe I was too fast? trying again soon...")
		 (sleep 5)
		 (get-uniprot-choice location t))))
	  (t (error "http get failed after choice (300), ~a" http-status)))))
(defun get-uniprot-fasta (uniprot-id &optional retrying)
  ...
  (multiple-value-bind (fasta-string http-status info)
    ...
    (cond ((= 200 http-status)
	   (fasta-combine-lines (split-sequence:split-sequence #\Newline fasta-string)))
	  ((= 404 http-status) (if retrying
				   (error "not found, even after waiting patiently and retrying")
				   (progn
				     (format t "  whoops... 404. maybe I was too fast? trying again soon...")
				     (sleep 3)
				     (get-uniprot-fasta uniprot-id t))))
	  ((= 300 http-status)
	   (get-uniprot-choice (cdr (assoc :location info))))
